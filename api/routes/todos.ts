...
router.get('/', async (req: AuthRequest, res: Response) => {
  try {
    const [todos] = await db.execute('SELECT * FROM todos WHERE user_id = ? ORDER BY created_at DESC', [req.userId!]);
    const [tags] = await db.execute('SELECT tt.* FROM todo_tags tt JOIN todos t ON tt.todo_id = t.id WHERE t.user_id = ?', [req.userId!]);
    const [subtasks] = await db.execute('SELECT s.* FROM subtasks s JOIN todos t ON s.todo_id = t.id WHERE t.user_id = ?', [req.userId!]);
    const [todoFiles] = await db.execute('SELECT tf.* FROM todo_files tf JOIN todos t ON tf.todo_id = t.id WHERE t.user_id = ?', [req.userId!]);
    const [todoNotes] = await db.execute('SELECT tn.* FROM todo_notes tn JOIN todos t ON tn.todo_id = t.id WHERE t.user_id = ?', [req.userId!]);
  ...
  try {
    ...
    if (tags && tags.length > 0) {
      for (const tag of tags) {
        await db.execute('INSERT OR IGNORE INTO todo_tags (todo_id, tag) VALUES (?, ?)', [id, tag]);
      }
    }

    if (fileIds && fileIds.length > 0) {
      for (const fileId of fileIds) {
        await db.execute('INSERT OR IGNORE INTO todo_files (todo_id, file_id) VALUES (?, ?)', [id, fileId]);
      }
    }

    if (subtasks && subtasks.length > 0) {
      for (const sub of subtasks) {
        await db.execute('INSERT INTO subtasks (id, todo_id, title, done) VALUES (?, ?, ?, ?)', [sub.id, id, sub.title, sub.done || false]);
      }
    }
    ...
    if (noteIds && noteIds.length > 0) {
      for (const noteId of noteIds) {
        await db.execute('INSERT OR IGNORE INTO note_todos (note_id, todo_id) VALUES (?, ?)', [noteId, id]);
        await db.execute('INSERT OR IGNORE INTO todo_notes (todo_id, note_id) VALUES (?, ?)', [id, noteId]);
      }
    }
